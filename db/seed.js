...
/* Data Declarations */
const PEOPLE = [
  ["Donna Chen", "donna@taskforge.io", "#C4623D"], ["Marcus Reed", "marcus@taskforge.io", "#5B7B9A"],
  ["Priya Nair", "priya@taskforge.io", "#7A8B6F"], ["Leo Park", "leo@taskforge.io", "#D89B4A"],
  ["Sana Okonkwo", "sana@taskforge.io", "#B5566B"], ["Theo Vance", "theo@taskforge.io", "#6B8E5A"],
  ["Iris Kaminski", "iris@taskforge.io", "#8A6BA8"], ["Noah Ferreira", "noah@taskforge.io", "#4F7A78"],
  ["Ruth Blackwood", "ruth@taskforge.io", "#A8763E"], ["Kenji Sato", "kenji@taskforge.io", "#3F6B8C"],
  ["Amara Diallo", "amara@taskforge.io", "#9C5A6B"]
];

const MERIDIAN_PROJECTS = [
  ["WEB", "Website Relaunch", "#C4623D", ["Review"]], ["MOB", "Mobile App v2", "#5B7B9A", ["QA"]],
  ["BPI", "Brand & Identity", "#7A8B6F", []], ["INF", "Infrastructure", "#4F7A78", ["Review"]],
  ["RES", "User Research", "#8A6BA8", []]
];

const ROLES = [
  [1, "admin"], [2, "member"], [3, "viewer"], [4, "member"], [5, "member"],
  [6, "member"], [7, "member"], [8, "viewer"], [9, "admin"], [10, "member"]
];
...
async function main() {
  await query("BEGIN");
  try {
    /* 1. Seed Users concurrently */
    const users = await Promise.all(
      PEOPLE.map(([name, email, color]) => createUser({ name, email, password: "password123", color }))
    );

    /* 2. Seed Org and Members concurrently */
    const org = await createOrg({ name: "Studio Meridian", slug: "meridian", createdBy: users[0].id });
    await Promise.all(ROLES.map(([uIdx, role]) => addMember({ orgId: org.id, userId: users[uIdx].id, role })));

    /* 3. Seed Projects & Fix Custom Column Placement */
    const projectsMap = {};
    for (const [key, name, color, extras] of MERIDIAN_PROJECTS) {
      const p = await createProject({ orgId: org.id, name, key, color });
      
      // Calculate inline modifications instead of pulling position max recursively
      if (extras.length > 0) {
        await query(`UPDATE columns SET position = position + $1 WHERE project_id = $2 AND name = 'Done'`, [extras.length, p.id]);
        await Promise.all(extras.map((extra, idx) => 
          query(`INSERT INTO columns (project_id, name, position) VALUES ($1, $2, $3)`, [p.id, extra, 2 + idx])
        ));
      }
      
      const cols = await listColumns(p.id);
      projectsMap[key] = { ...p, cols: Object.fromEntries(cols.map(c => [c.name, c.id])) };
    }

    /* 4. Seed Tasks concurrently */
    const tasks = await Promise.all(TASKS.map(([key, col, title, priority, who, due, desc]) => 
      createTask({
        projectId: projectsMap[key].id,
        columnId: projectsMap[key].cols[col],
        title,
        description: desc || "",
        priority,
    ...
    const tasksMap = Object.fromEntries(tasks.map(t => [t.title, t]));

    /* 5. Seed Activity & Notifications concurrently */
    await Promise.all([
      ...COMMENTS.map(([title, who, body]) => addComment({ taskId: tasksMap[title].id, userId: users[who].id, body })),
      ...FILES.map(([title, who, filename, sizeBytes]) => addAttachment({ taskId: tasksMap[title].id, userId: users[who].id, filename, sizeBytes })),
      ...NOTIFS.map(([uIdx, body, title]) => createNotification({ userId: users[uIdx].id, body, taskId: tasksMap[title]?.id ?? null }))
    ]);

    /* 6. Mark first 2 notifications for Donna as read */
    await query(`
      UPDATE notifications SET is_read = true 
      WHERE id IN (SELECT id FROM notifications WHERE user_id = $1 ORDER BY id LIMIT 2)
    `, [users[0].id]);
