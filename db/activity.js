...
export const deleteAttachment = async (id) =>
  (await affected(`DELETE FROM attachments WHERE id = $1`, [id])) > 0;

/* ---------------------------- notifications -------------------------------- */

// A notification is served only while its recipient can still see the task it
// points at: rows referencing a task in an org the user no longer belongs to
// are filtered out rather than leaking titles across workspaces.
...
export const markAllRead = (userId) =>
  affected(`UPDATE notifications SET is_read = true WHERE user_id = $1`, [userId]);
